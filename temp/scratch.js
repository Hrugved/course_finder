...
db.query(query_setup, function (err, _) {
  ...
  readXlsxFile('./Course_Schedule_2021-22-2-converted.xlsx').then((rows) => {
    ...
    rows.forEach(row => {
      ...
      // console.log(credits);
      let course_types = row[5].toUpperCase().split(/,|\//); // comma or forward-slash
      course_types = course_types.map(raw => raw.replace(/\s/g, '').trim());
      course_types_bitmap = getCourseTypesBitmap(course_types);
